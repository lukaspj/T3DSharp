...
namespace T3DSharpFramework.Interop {
   internal class EngineCallbacks {
      ...
      private static string CallNamespaceMethod(Type namespaceClass, ISimObject objectBaseWrapper, string methodName,
         ...
         if (callbackMethod != null) {
            ...
            if (!callbackMethod.IsStatic) {
               // If we don't have the wrapper object, it will be passed in as the "this" parameter.
               objectBaseWrapper ??= Sim.FindObjectById<ISimObject>(Convert.ToUInt32(args.First().ToString()));

               // It's a named object, we just need to change the type
               simObj = (ISimObject)SimDictionary.CreateInstance(namespaceClass, objectBaseWrapper);
            }

            return InvokeMethod(callbackMethod, simObj, args.Skip(1).ToArray(), out found);
         }
      ...
      public static Dictionary<string, MethodInfo> GetFunctions() {
         return FunctionDictionary;
      }

      public static Dictionary<string, Type> GetTypes() {
         return ClassTypeDictionary;
      }
   }
}
